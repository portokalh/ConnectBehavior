...
lasso_model <- cv.glmnet(image, riskfactors$NormSWDist, alpha = 1)
plot(lasso_model)



print(lasso_model) # could do print(out,verbose=TRUE)
#print(image[out$u!=0]) 

u=coef(lasso_model, s = lasso_model$lambda.min)


##########
# u[abs(u)<quantile(u,0.999)]  = 0
# nonzerou = u[u!=0]
# nonzerou[abs(nonzerou)<quantile(nonzerou,0.99)]   = 0 
# u[u!=0] = nonzerou
#######
v=out$v
